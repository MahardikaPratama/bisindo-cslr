...
Command-line usage (quick):
    ...
    python visualizer/plot_skeleton.py data/skeleton/00_0001.npy 40
...
def plot_frame(keypoints: np.ndarray,
    ...
    if scale_to is not None:
        ...
        if np.nanmax(coords) <= 1.0 + 1e-6:
            ...
            logger.warning(
                "plot_skeleton.plot_frame: coordinates look like pixel values (>1). Applying scale_to may produce incorrect results."
            )
            coords = coords.copy()
            coords[:, 0] = coords[:, 0] * float(w)
            coords[:, 1] = coords[:, 1] * float(h)
