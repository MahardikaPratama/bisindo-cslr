...
const VideoInputPanel = React.memo(function VideoInputPanel(_props: VideoInputPanelProps) {
  ...
  return (
    <Card className="flex flex-col gap-4">
      ...
      <div
        className={cn(
          'relative w-full aspect-video rounded-lg border-2 border-dashed flex flex-col items-center justify-center overflow-hidden',
          videoStatus !== 'idle'
        ...
        {videoObjectUrl && videoStatus !== 'idle' ? (
          <>
            ...
            <div className="relative z-10 flex flex-col items-center gap-2 p-4 text-center">
              ...
              <span className="text-xs text-text-secondary">Siap diterjemahkan</span>
            </div>
          </>
        ) : (
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-surface-panel-2 rounded-lg py-2.5 px-3 flex flex-col items-center justify-center border border-surface-border/50">
          <span className="text-[10px] text-text-secondary font-medium uppercase tracking-wider mb-0.5">Durasi</span>
          <span className="text-sm font-semibold text-text-primary font-mono">
            {videoMetadata?.duration ? `${formatDuration(videoMetadata.duration)}s` : '-'}
          </span>
        </div>
        <div className="bg-surface-panel-2 rounded-lg py-2.5 px-3 flex flex-col items-center justify-center border border-surface-border/50">
          <span className="text-[10px] text-text-secondary font-medium uppercase tracking-wider mb-0.5">Resolusi</span>
          <span className="text-sm font-semibold text-text-primary font-mono">
